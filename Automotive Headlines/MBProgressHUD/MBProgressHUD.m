...
- (void)updateIndicators {
	// 改动
//	BOOL isActivityIndicator = [indicator isKindOfClass:[LoadingView class]];
...
//    self.color = nil;
	if (mode == MBProgressHUDModeIndeterminate) {
		if (!isActivityIndicator) {
            ...
            // 改动
//            self.indicator = [[LoadingView alloc] initWithFrame:CGRectMake(0, 0, 100, 100)];
...
- (void)layoutSubviews {
	...
	CGSize labelSize = MB_MULTILINE_TEXTSIZE(label.text, label.font, CGSizeMake(maxWidth, bounds.size.height), label.lineBreakMode);
